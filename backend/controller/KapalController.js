...
class KapalController {
  ...
  static async create(req, res, next) {
    try {
      const { perusahaan_id, nama_kapal, berat_kapal } = req.body;
      
      if (!perusahaan_id || !nama_kapal) {
        return res.status(400).json({ error: 'Perusahaan dan nama kapal harus diisi' });
      }

      const data = await KapalModel.create(perusahaan_id, nama_kapal, berat_kapal);
  ...
  static async update(req, res, next) {
    try {
      ...
      const data = await KapalModel.update(req.params.id, perusahaan_id, nama_kapal, berat_kapal);
