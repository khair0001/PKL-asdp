...
const isEmpty = computed(() => {
  return !loading.value && (revenueData.value.length === 0 || revenueData.value.every(item => item.total === 0));
});
...
const renderChart = async () => {
  ...
  try {
    ...
    const data = revenueData.value.map(item => item.total);
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              ...
              label: (context) => {
                return `Pendapatan: ${formatTooltipCurrency(context.parsed.y)}`;
              }
            },
